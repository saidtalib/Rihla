...
class _SocialChatScreenState extends State<SocialChatScreen>
  ...
  // ── Attachment picker (image or PDF) ────────
  ...
  Future<void> _pickImage() async {
    ...
    try {
      await ChatService.instance
          .sendPhoto(widget.trip.id, File(file.path));
      _scrollToBottom();
    } catch (e) {
      if (!mounted) return;
      ...
      children: [
            ...
            builder: (context, snap) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, i) {
                  ...
                  final canDelete =
                      isMe || _isAdmin; // admins can delete any
                  return _ChatBubble(
                    message: msg,
                    isMe: isMe,
                    isDark: dark,
                    ...
                    onOpenFile: msg.hasFile ? () => _openFile(msg.fileUrl!) : null,
                  );
                },
              );
            },
        ...
        if (_uploading)
          const LinearProgressIndicator(
            ...
            backgroundColor: RihlaColors.saharaSand,
          ),

        // ── Input bar ─────────────────────────
        _buildInputBar(ar, dark, fontFamily!),
      ],
    );
  }

  Widget _buildInputBar(bool ar, bool dark, String fontFamily) {
    return Container(
          ...
          IconButton(
            icon: _uploading
                ...
                : const Icon(Icons.add_circle_rounded,
                    color: RihlaColors.sunsetOrange, size: 28),
...
class _ChatBubble extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Colors: sender = Jungle Green, others = Sahara Sand
    final bgColor = isMe
        ? RihlaColors.jungleGreen.withValues(alpha: 0.85)
        : (isDark
            ? RihlaColors.darkCard
            : RihlaColors.saharaSand.withValues(alpha: 0.9));
                    ...
                    if (message.isPdf)
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: (isMe ? Colors.white : RihlaColors.jungleGreen)
  ...
  Widget _buildAvatar() {
    ...
    if (photoUrl != null && photoUrl.isNotEmpty) {
      return CircleAvatar(
        ...
        backgroundColor: RihlaColors.saharaSandDark,
      );
    }
    // Default safari explorer icon
    return CircleAvatar(
      radius: 16,
      ...
      child: Icon(
        ...
        size: 18,
        color: isMe ? RihlaColors.jungleGreen : RihlaColors.sunsetOrange,
      ),
    );
  }
}
